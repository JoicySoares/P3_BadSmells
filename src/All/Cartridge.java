...
class Cartridge {
	...
	public Cartridge(String romFileName, Component a) {
		...
		try {
			...
			do { // Read ROM into memory
				total -= is.read(rom, rom.length - total, total); // Read the
																	// entire
																	// ROM
			} while (total > 0);
			is.close();
			...
			if (!verifyChecksum() && (a instanceof Frame)) {
				new ModalDialog((Frame) a, "Warning",
	...
	public InputStream openRom(String romFileName, Component a) {
		...
		String romName = "Pokemon";
		romFileName = "/home/joicy/Área de Trabalho/JavaBoy/Rom/pokemon_-_blue_version.zip";
...
class WebSaveRAM implements Runnable, DialogListener {
	...
	public WebSaveRAM(URL url, boolean save, Cartridge cart, Dmgcpu cpu,
		...
		if (!cart.canSave()) {
			...
			ModalDialog d = new ModalDialog(null, "Sorry",
					"This game does not", "have a save facility.");

		} else {

			if (save) {
				ModalDialog d = new ModalDialog(null, "Confirm",
